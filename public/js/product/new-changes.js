$('#plus_bt').click(function() {
	var pri = parseInt($('.item-price').attr('content'));
	var bt = $('#count_bt').val();
	if(parseInt(bt) >= 100) {
		$('#count_bt').val(100);
		$('.item-price').html((pri*100).toLocaleString());
		$("#butnmj").attr("data-request-data", "tov_id: "+ $("#butnmj").attr("data-product-id") +", tov_kol: '100'");
	}
	else {
		$('#count_bt').val(parseInt(bt)+1);
		$('.item-price').html((pri*(parseInt(bt)+1)).toLocaleString());
		$("#butnmj").attr("data-request-data", "tov_id: "+ $("#butnmj").attr("data-product-id") +", tov_kol: '"+(parseInt(bt)+1)+"'");
	}
});
$('#minus_bt').click(function() {
	...
	if(parseInt(bt) <= 1) {
		$('#count_bt').val(1);
		$('.item-price').html((pri).toLocaleString());
		$("#butnmj").attr("data-request-data", "tov_id: "+ $("#butnmj").attr("data-product-id") +", tov_kol: '1'");
	}
	else {
		$('#count_bt').val(parseInt(bt)-1);
		$('.item-price').html((pri*(parseInt(bt)-1)).toLocaleString());
...
$('#count_bt').on('input keyup', function() {
	var pri = parseInt($('.item-price').attr('content'));
	var bt = $('#count_bt').val();
	if(parseInt(bt) >= 100) {
		$('#count_bt').val(100);
		$('.item-price').html((pri*100).toLocaleString());
		$("#butnmj").attr("data-request-data", "tov_id: "+ $("#butnmj").attr("data-product-id") +", tov_kol: '100'");
	} else if(parseInt(bt) <= 1) {
		$('#count_bt').val(1);
		$('.item-price').html((pri).toLocaleString());
		$("#butnmj").attr("data-request-data", "tov_id: "+ $("#butnmj").attr("data-product-id") +", tov_kol: '1'");
	} else {
		$('.item-price').html((pri*parseInt(bt)).toLocaleString());
		$("#butnmj").attr("data-request-data", "tov_id: "+ $("#butnmj").attr("data-product-id") +", tov_kol: '"+bt+"'");
	}
});
...
document.querySelector(".button-basket-outside .counter-value .counter-postfix").innerHTML = wordDecline($.Cart.getCount(), ['товар','товара','товаров']);
}
else {
document.querySelector('.button-basket-outside .cartReplacerCount').innerText = $.Cart.getCount(); 
}
...
function cloneNodes() {
const copyNodes = document.querySelector('.product-slider-nav-images-container').cloneNode(true);
[...copyNodes.children].forEach( el =>
document.querySelector('.product-slider-nav-images-container').appendChild(el));
}
} 
...
[...document.querySelectorAll(".details-block > input[type='radio']")].forEach( el => el.setAttribute("type","checkbox")
);